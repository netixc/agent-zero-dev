...
export function _drawMessage(
  ...
  // Auto-collapse agent and tool messages based on preference
  const autoCollapse = localStorage.getItem("autoCollapse") === "true";
  if (autoCollapse && (messageClasses.includes("message-agent") || messageClasses.includes("message-tool"))) {
...
// Function to toggle message collapse/expand
export function toggleMessageCollapse(messageDiv) {
  const isCollapsed = messageDiv.classList.contains("collapsed");
  const collapseButton = messageDiv.querySelector(".collapse-button");
