...
// Make toggleMessageCollapse available globally
window.toggleMessageCollapse = msgs.toggleMessageCollapse;
...
export async function sendMessage() {
  try {
    ...
    if (window.speech) {
      await window.speech.enableAudio();
    }
...
function setMessage(id, type, heading, content, temp, kvps = null) {
  ...
  // Check for TTS trigger in setMessage
...
function checkTTSInSetMessage(id, type, heading, content, temp, kvps) {
  ...
  // Only process TTS if user has interacted with the page (audio enabled)
  if (!window.speech?.audioEnabled) {
    return;
  }

  console.log(`setMessage: id=${id} type=${type} heading="${heading}" content="${content}" temp=${temp} kvps=`, kvps);

  // Look for agent responses that should be spoken
  if (type === "response" && !temp && content && content.trim()) {
    // Log link detection but don't skip - we'll clean the content instead
    ...
    // Initialize or get existing state for this response
    if (!responseStates.has(id)) {
      // New response starting - stop current TTS and clear queue
      stopCurrentTTS();
      clearTTSQueue();
    ...
    // Clean the content - keep link text but remove URLs
    ...
    // Get the new text chunk that we haven't processed yet
    const newChunk = cleanContent.substring(state.processedText.length);
    
    if (newChunk.length > 0) {
      // Add new chunk to current sentence
      ...
      const sentences = state.currentSentence.split(/([.!?]+)/);
      let processedText = "";
      let remainingText = "";
      ...
      for (let i = 0; i < sentences.length - 1; i += 2) {
        ...
        if (sentenceText && punctuation && /[.!?]/.test(punctuation)) {
          const completeSentence = (sentenceText + punctuation).trim();
          
          // Only speak if sentence is substantial and meaningful
          if (completeSentence.length > 15 && !/^[\s\n\r]*$/.test(sentenceText)) {
            queueTTS(completeSentence);
          }
          
          processedText += completeSentence;
        }
      }
      
      // Keep any remaining incomplete text
      if (sentences.length > 0) {
        const lastIndex = Math.floor((sentences.length - 1) / 2) * 2;
        remainingText = sentences.slice(lastIndex).join("").trim();
      }
      
      state.currentSentence = remainingText;
      
      // Update processed text
      state.processedText = cleanContent;
    }
    
    // Check if this might be the final update (no more streaming)
    // We'll handle remaining content with a timeout
    setTimeout(() => {
      if (state.currentSentence.trim() && !state.isComplete) {
        const remainingText = state.currentSentence.trim();
        if (remainingText.length > 5) { // Speak remaining text if substantial
    ...
    }, 1500); // Wait 1.5 seconds for response to complete
  }
}
...
function stopCurrentTTS() {
  ...
  if (window.speech && window.speech.currentAudio) {
    try {
      window.speech.currentAudio.pause();
      window.speech.currentAudio.currentTime = 0;
    } catch (error) {
    }
  }
...
async function processQueue() {
  ...
  const timeoutId = setTimeout(() => {
    console.warn("*** TTS: Timeout reached, forcing queue unlock");
    isPlaying = false;
    processQueue();
...
function speakAndWait(text) {
  return new Promise((resolve, reject) => {
    ...
    speech.speak(text).then(() => {
      // Audio started playing, now wait for it to finish
      if (speech.currentAudio) {
        speech.currentAudio.onended = () => {
          resolve();
        };
        speech.currentAudio.onerror = () => {
          console.error("TTS: Queue item playback error");
          reject();
        };
      } else {
        // No audio was created, resolve immediately
        resolve();
      }
    }).catch(reject);
  });
...
window.toggleAutoCollapse = function (isOn) {
  console.log("Auto-collapse tools:", isOn);
  localStorage.setItem("autoCollapse", isOn);
  
  // Apply/remove collapsed state to existing messages
  const agentMessages = document.querySelectorAll(".message-agent, .message-tool");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Initialize auto-collapse preference
